...
public class ProjectManagerTests
{
    ...
    private static readonly string _referenceBrokenPath =
        Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)
        + @"\TestData\ReferenceBroken.txt";

    private static readonly string _nonexistentFile =
        Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)
        + @"\TestData\NonexistentFile.txt";
    ...
    public void TestReadProject_CorrectData()
    {
        ...
        var expected = JsonConvert.DeserializeObject<Project>(expectedString);
        if (File.Exists(_path))
        {
            File.Delete(_path);
        }
    ...
    public void TestReadProject_NonexistentFile()
    {
        ...
        var actual = JsonConvert.SerializeObject(ProjectManager.ReadProject());
    ...
    public void TestSaveProject_WithCreatedFile()
    {
        ...
        {
            var actualString = File.ReadAllText(_path);
            var actual = JsonConvert.DeserializeObject<Project>(actualString);
            Assert.AreEqual(
                expectedString,
                actualString,
                "Values are not the same");
    ...
    public void TestSaveProject_WithoutCreatedFile()
    {
        ...
        {
            var actualString = File.ReadAllText(ProjectManager.DefaultPath);
            var actual = JsonConvert.DeserializeObject<Project>(actualString);
            Assert.AreEqual(
                expectedString,
                actualString,
